...
function setupUI(): void {
  ...
  controls.innerHTML = `
    ...
    <div style="display: flex; gap: 5px; margin-top: 8px;">
      <select id="exploration-policy-select" style="width: 100%; padding: 5px; background: #0a1a2a; color: #00ffdd; border: 1px solid #00ffdd;">
        <option value="/models/dyna_q_policy.json">Dyna-Q (default)</option>
        <option value="/models/hybrid_drl_explorer_policy.json" selected>Hybrid DRL</option>
        <option value="/models/vanilla_q_policy.json">Vanilla Q</option>
      </select>
    </div>
  ...
  const policyChoices = [
    { label: 'Dyna-Q (default)', path: '/models/dyna_q_policy.json' },
    { label: 'Hybrid DRL', path: '/models/hybrid_drl_explorer_policy.json' },
    { label: 'Vanilla Q', path: '/models/vanilla_q_policy.json' },
  ];
  ...
  async function refreshPolicies() {
    try {
      ...
      const predefined = [
        ...
        { label: 'Vanilla Q (vanilla_q_policy)', path: '/models/vanilla_q_policy.json' },
      ];
